    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["movie-poster"]["tmp_name"], $target_file)) {
            ...
            if (mysqli_query($conn, $sql_movie) && mysqli_query($conn, $sql_show)) {
                echo "<script>alert('New movie added successfully'); window.location.href='staff-dashboard.php';</script>";
            } else {
    ...
    <div class="card-body">
        <form id="add-movie-form" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group d-flex justify-content-between">
                <button type="button" class="btn btn-secondary" onclick="window.location.href='staff-dashboard.php'" style="background-color: #5b0f0f ; border-radius: 20px; padding: 10px 25px;">Cancel</button>
                <button type="submit" class="btn btn-primary" style="background-color: #5b0f0f; border-radius: 20px; padding: 10px 25px;" onclick="window.location.href='staff-dashboard.php'">Add Movie</button>
            </div>
        </form>
    </div>
</div>
